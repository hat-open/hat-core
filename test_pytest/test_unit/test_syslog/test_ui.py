...
@pytest.fixture
async def web_server(ui_address, ui_path, backend):
    web_server = await hat.syslog.server.ui.create_web_server(
        ui_address, None, ui_path, backend)
    try:
        yield web_server
    finally:
...
async def test_get_static_files(web_server, ui_path, ui_address):
    with open(ui_path / 'index.html', 'w', encoding='utf-8') as f:
        f.write('123')

    async with aiohttp.ClientSession() as session:
        async with session.get(ui_address + '/index.html') as resp:
            assert resp.status == 200
            assert '123' == (await resp.text())

    async with aiohttp.ClientSession() as session:
        async with session.get(ui_address) as resp:
            assert resp.status == 200
            assert '123' == (await resp.text())

    async with aiohttp.ClientSession() as session:
        async with session.get(ui_address + '/abc.txt') as resp:
